...
const state = reactive<{
  showChat: boolean;
  isLoggedIn: boolean;
  showMemoryChat: boolean;
}>({
  showChat: false,
  isLoggedIn: false,
  showMemoryChat: false,
});
...
<template>
  <div
    v-if="!state.showChat && !state.showMemoryChat"
    class="w-full h-screen flex items-center justify-center flex-col gap-4"
  >
    <button
      type="button"
    ...
    <button
      ...
      type="button"
      class="text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-full text-base px-6 py-3 text-center"
      @click="state.showMemoryChat = true"
    >
      Memory Chat
    </button>
    <button
  ...
  <div v-if="state.showChat"><Chat /></div>
  <div v-if="state.showMemoryChat"><MemoryChat /></div>
</template>
